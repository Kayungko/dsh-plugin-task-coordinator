...
6. **设置页签占用 [0.18.0]**：`settings.plugins.tab` 槽（dsh-community-market 第三方先例）注册 `id: "task-coordinator"` 页签——设置 → 插件 → 「任务编排」。数据面：候选经 `ctx.remote.session.modelCatalog()`（GUI 选择器同源，宿主目录含自建网关路线时自动出现）；读写经 `ctx.settingsScope.bind({namespace: 'task-coordinator'})`（`getSnapshot/subscribe` 跟随 describe 镜像，`set(field, value)` 三次队列写回 provider/model/reasoningEffort）。两者沿用 0.16.1 懒服务纪律（bundle 仍只 inject `["slots"]`，按需防御寻视 + ~32s 重试封顶 + 降级文案）；已保存但目录下线的路线显示「（已下线）」仍可改选（对齐原生 subagent-model-selection 卡 stored/available 合并语义）。
