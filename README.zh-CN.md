...
会话头部的第三个页签「**编排**」（排在原生 聊天/轨迹 之后，order 20）：这是纯监督工作台，进入时隐藏 composer 与左右宽度拖拽手柄，离开后恢复；任务画布独立滚动，详情以非模态右侧悬浮抽屉呈现（窄屏同样悬浮，不降到拓扑下方），抽屉高度不超过可视区域且内容独立滚动。把当前会话作为总控，实时渲染它派发的全部子任务——总控节点卡顶部居中，子会话卡片按 team 分组成行排在下方（无 team 归「未编组」行）；派发（实线下行）、指令（steer/queue 强调色下行、带模式标签）、汇报（虚线上行）三类**方向性连线**一目了然；**2 分钟内有活动的连线加流光动画**（0.26.0 起运行节点不再使用呼吸脉冲）；版心使用实际会话面板宽度、上限 1440px（**0918 用户裁决追认**：0.22.3 的聊天列宽绑定 `--dsh-chat-content-width` 废止——编组并排布局需要宽度；历史沿革见 CHANGELOG 0.26.0）。点击任务卡片打开或切换右侧非模态详情抽屉，再次点击同一卡片、按 Esc 或点关闭按钮收起；抽屉展示状态/待办/派发模型/往来记录且画布保持全宽，点「打开会话」才跳转。0.21.x 曾改为泳道时间轴形态，用户真机裁决后 0.22.0 回归卡片拓扑（方向感与驻留感更合监督心智），并保留泳道时代的全部改进。
