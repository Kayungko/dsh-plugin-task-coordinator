...
const STRINGS = {
  zh: Object.freeze({
    ...
    reportBackSuffix: (callerSessionId) => `汇报约定：完成（或确认无法完成）后，用 task_send 把结果摘要发回会话 ${callerSessionId}（内容：结论、产出路径、遗留问题）。发送后即结束当前回合——不要在回合内等待或轮询总控的回复，它会在你空闲时自动开新轮送达。多阶段任务：需总控评审的阶段，在该阶段结束时发阶段报告并结束回合等待指示；未约定评审点的阶段连续执行。若发送失败，把摘要完整写进你的最终回复。注意：汇报只能用 task_send——宿主自带的 send_message 仅适用于 subagent 树子代理，对 task_spawn 的登记父子会被宿主拒绝（belongs to another parent）。`,
