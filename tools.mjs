...
export function registerTools(ctx, ops, deps, config) {
  ...
  disposers.push(ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      externalRef: { type: 'string', description: 'Optional external caller reference for cross-agent correspondence: a free-form text label (suggested shape "<thread-short-id>:<wave-name>") supplied by an external dispatcher (e.g. a Codex conversation driving DSH through the task bridge) so it can later answer "which of my conversations/waves dispatched this session". Trimmed, at most 200 characters (empty/whitespace-only counts as absent; a non-string or over-length value is rejected bad-request). Stored durably in the spawn registry and echoed on the receipt, task_list rows and task_progress — never parsed.' },
