...
### 文档：启用梯度门槛提到 bridge-mcp ≥0.5.4，并更正一处**在当时就是假的**安全声明

纯文档版 + 版本号，插件代码与 wire 契约零改动。

上一版（0.27.5）把模式 B 的启用梯度按 bridge-mcp 0.5.2 重写，写的是「≥0.5.2」。但 0.5.2 带着
一个后来被定为 **P1** 的缺陷：0.5.1 为还原回执里 OS 正确大小写，给 `displayPath` 加了「叶子不存在
时逐级上溯到最近存在祖先做 realpath」，却**没有**同步给 `canonical`——后者 realpath 失败时退回纯
字符串 `resolve()`，不解析祖先链接。而 `guardPath` 用 `canonical` **判定**、`writeFile` 用
`displayPath` **落盘**，于是「junction/symlink 祖先 + 不存在的叶子」让两者指向不同物理位置：
**检查在一个路径上、写入在另一个路径上**。实测可穿透全部三级凭据保护，含文档写明「不可通过任何
配置解除」的那一级；且 Windows 自带 `C:\Documents and Settings` → `C:\Users` 的 junction，
**零前置条件**可利用（`~/.ssh/authorized_keys` 通常不存在，恰好满足触发前提）。

所以「≥0.5.2」这个门槛本身就是错的——照它配置的用户会以为自己有六层防护，实际最里面那层可绕。
本版把门槛提到 **≥0.5.4**（0.5.3 修掉 P1，0.5.4 补齐同批交叉校验的四项），并在文件头的
「适用版本」行同步：`local_*` 工具需 ≥0.5.0，但**开 `local_*` 需 ≥0.5.4**。

同时更正第 2 档里那句「`..` 穿越与 junction 逃逸都以 canonical 结果判定」。这句在 0.5.2 及以前
对**新建文件**是**假的**（正是上面那个 P1），此前既有实测覆盖的是**已存在**的叶子。现在原文保留
但加了版本限定与成因说明，避免读者以为它一直成立。

另补一条 0.5.4 带来的行为变化，因为它会**影响可用性**而不只是安全：凭据保护清单里的名字
（`credentials` / `.env` / `auth.json` 等）用作**目录名**时，现在连带保护整个子树。此前只测路径
自身的 basename，于是一个名为 `auth.json` 的目录自身受保护（遍历跳过它）、但其子文件判为不受保护
（`guardPath` 放行直读）——同一条路径「直读能读、遍历搜不到」。代价是：项目里真有这类同名目录
（测试 fixture、mock 数据、第三方仓常见）时，其子树会一并不可访问，需
`DSH_BRIDGE_FS_DENY_CREDENTIALS=0` 解除（启动打强告警）。这一层仍是**可解除**的，没有新增
「不可解除」级别——那一级只留给本链路自己的凭据。

还有一处**回执语义变更**需要下游注意（属 bridge-mcp 0.5.4，但会影响读回执的调用方）：
`local_list_dir` 的 `truncated` 从「被 limit 截断」改为「**结果不完整**」（任何原因），狭义标志
改名 `limitTruncated`，与 `local_grep` 的既有约定对齐。此前不可读子目录是静默跳过，
`truncated:false` 会让调用方把**不完整的树**读成完整。若插件侧或用户脚本有按旧语义读
`truncated` 的地方，需改看 `limitTruncated`。

## [0.27.5] - 2026-09-24

### 文档：模式 B 的启用梯度按 bridge-mcp 0.5.2 重写

纯文档版，插件代码与 wire 契约零改动。

bridge-mcp 0.5.2 修掉了它自己「已知未修」清单里的三条（ReDoS 冻结 server、默认凭据保护清单漏项、
`local_write_file` 可改写自身源码），并新增了白名单模式 `DSH_BRIDGE_FS_ALLOW`。
本文的「建议的启用梯度」原来只有两档（先只开文件、确需再开 shell），既没提白名单，
也没把「开了 exec 之后文件级防护还剩多少」说清楚，于是照原文配置的用户会**高估**自己受到的保护。

改为三档，并写明每档的真实边界：

1. 只开文件（`DSH_BRIDGE_LOCAL_FS=1`）；
2. 同时用 `DSH_BRIDGE_FS_ALLOW=<项目根>` 把范围收窄到项目目录——白名单与凭据保护是 **AND**
   关系（只收窄、永不放宽：把 home 加进白名单，`~/.ssh/id_rsa` 与桥 token 照样被拒），
   且递归遍历内逐条目生效，`..` 穿越与 junction 逃逸都以 canonical 结果判定；
3. 确需跑命令再开 `DSH_BRIDGE_LOCAL_EXEC=1`，并明确：**开了 exec 就没有文件级边界了**。
   凭据保护、白名单、自身完整性保护全都是文件工具层的约束，shell 不受它们管。

第 3 档那句不是推断：bridge-mcp 0.5.2 用独立脚本真跑了一次 `echo … > <受保护路径>`
（用注入的假包根演示，未触碰真实源码），确认写入成功。自身完整性保护那层同样实测过
作用域——它真正防的是「只开文件不开 shell」这个更窄配置下的唯一改写通道，而那恰好是本文
推荐的第 1、2 档。

另补两条使用纪律：`local_write_file` 不得改写 bridge-mcp 自己的包目录（生产 profile 直接跑
工作树，改写源码会在下次重启后被加载，属持久化通道）与审计日志文件（前者可用
`DSH_BRIDGE_FS_ALLOW_SELF_WRITE=1` 解除，后者不可解除）；审计行已做防注入编码，
但**命令原文会逐字落盘**，所以内联了秘密的命令会把秘密写进审计文件——
`DSH_BRIDGE_AUDIT_FILE` 要按凭据来管，别放在会被 `local_grep` 扫到的项目目录里。

「四层防护」的表述同步改为「六层」（0.5.2 新增白名单与自身完整性两层）。
...
- 修复 v0.26.4 引入、v0.26.5 未修完的抽屉宽度回归：宽面板（容器 >900px）下编排详情抽屉完全不可见。根因是 `.orchViewInspector` 基础规则仍用 `width:min(380px,calc(100% - 32px))`，而 0.26.4 起其包含块改为零宽 sticky 哨兵（`.orchDrawerLayer{width:0}`），百分比宽度按 0 解析得负值被钳制为 0（窄屏 ≤900px 因容器查询覆盖已用 `100cqw` 而幸免）。基础规则宽度改为 `min(380px,calc(100cqw - 32px))`，与窄屏覆盖一致，宽度基准回到容器 `orch`（`.orchViewRoot`）本身。
- 契约测试同步修正：0.26.5 的断言把错误的 `calc(100% - 32px)` 钉死导致带病出厂全绿；现改为钉住 `cqw` 写法，并新增禁止百分比宽度回潮的回归断言。
...
- 编排页点击任务画布空白区域可收起右侧详情抽屉；点击任务卡片仍负责打开/切换，抽屉内部点击不冒泡关闭，拖拽位移超过 5px 不误触发空白关闭，且收起时同步清除 selectedTaskId 与高亮。

## [0.26.2] - 2026-09-18

### 编排页监督化

- 编排页改为纯监督工作台：挂载时隐藏宿主 composer 与左右宽度拖拽手柄，卸载时精确恢复聊天布局。
- 任务画布独立滚动；详情抽屉位于独立覆盖层，非模态且固定在编排面板右侧，窄屏不再降到拓扑下方。
- 抽屉自适应可视高度并内部滚动；切换/重复点击、Esc、关闭按钮同步维护 `drawerOpen`、`selectedTaskId` 与高亮状态。
- 保留用户裁决 A：会话面板实际宽度、上限 1440px；未恢复聊天列宽绑定。

## [0.26.1] - 2026-09-18

### 抽屉化与评审修复

- 编排页详情改为右侧非模态抽屉：点击卡片打开/切换，再次点击收起，支持 Esc 与关闭按钮；画布保持全宽。
- 修复 family 历史首载在投影缺失时回退 `throughSeq:-1`，并为列表变化增加 1.5 秒防抖。
- 优化图标 URI 缓存与分组子任务计数，统一首帧加载空态。
...
- 从总控、子任务或孙任务进入编排页时，按持久注册表解析同一所属编排；定位当前任务，默认展开当前祖先分支，其他下级分支可折叠，提供“打开总控”和“定位当前任务”。
- 新增宿主 Connection 鉴权通道下的两个只读查询：所属编排关系与选中任务的父会话往来分页。只传结构和关系事实，不返回提示词、路径或外部引用。
- 明确区分查询不可用、父链不完整、注册表无关联与历史未加载；注册表未保留的旧任务仍可展示当前已加载的派发记录。
- 首次安装此功能需重新加载宿主插件；单独刷新浏览器不能创建新的宿主查询路由。

### 编排页界面（随 0.26.0 发布，09-12 波次 e15bd01）

- 实屏修正：分组按实际任务数量紧凑排布，1/2/2 个任务的三个编组可并排呈现；不再为小组保留空卡位。底部使用宿主动态 `--dsh-composer-height` 预留输入框空间，非选中关系弱化显示。
- 桌面/Web 一致性修正：编排页使用实际会话面板宽度，取消聊天正文栏的 680–920px 限制；背景、弱底色、强调色改用当前宿主实际定义的语义变量；客户端重复挂载时更新已有样式标签，避免沿用旧 CSS。
- 卡片拓扑按编组并排展示，按首次派发顺序排列；组内任务自动换行，窄窗口将详情移至图下方，跟随宿主可用面板宽度与主题。
- 点击任务卡片在页内选中，右侧查看状态、待办、派发时模型及本总控已加载的关系记录；点击“打开会话”才跳转，保留导航失败时的复制 ID 回退。
- 连线按编组和往来类型聚合；详情显示最近 20 条单独记录。失败指令单独标记，不计入成功连线。
- 区分历史未加载、确实未派发和记录服务不可用；无法读取实时会话状态时显示“状态未知”。
- 增加选择/跳转、会话切换、布局边界、失败投递、空态与分页恢复回归测试。

### Codex 协作增量（09-12 波次原文存档）

等待校验真实目标；增量消息游标与消息身份对账；外部分组投影及有界、可持久化派发/投递回执；未知归属和历史缺口保留明确标记。

> 收编注：原文「仅源码变更，不表示已部署、重启或完成实机验收」为 09-12 时点状态；该波次实际已于 09-12 晚部署、随 09-17 宿主重启在产运行，0918 收编定版为 0.26.0。
