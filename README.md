...
The third conversation tab, "**Orchestration**" (after the native Chat / Trajectory tabs, order 20): it anchors on the CURRENT session as the supervisor and renders everything it dispatched in real time — the supervisor node card centered on top, child session cards grouped into team rows below (no-team children fall into the Ungrouped row), and three kinds of **directional SVG edges**: spawn (solid, downward), message (steer/queue accent, downward, mode-labeled) and report-back (dashed, upward); **edges active within the last two minutes carry a dash-flow shimmer, running nodes breathe with a pulse**; the view's column matches the chat content width (0.22.3: bound to the host's `--dsh-chat-content-width` custom property — flush with the composer card's edges and live-following the conversation column's drag-resize handle with zero JS listeners; a narrow fleet's canvas content centers itself). 0.21.x briefly switched to a lane-timeline form; after living with it the user ruled cards more readable for supervision, so 0.22.0 restores the topology and keeps every improvement from the lane era.
